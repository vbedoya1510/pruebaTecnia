...
namespace WebApi.Model
{
    public class AdminUsers
    {
        ...
        public IEnumerable<Users> getUsersAll()
        {
            try
            {
                List<Users> lk = new List<Users>();

                List <Users> listUser = new List<Users>();

                using (TestJulius2Entities julius2DB = new TestJulius2Entities())
                {
                    var usuarios = julius2DB.Users.ToList();

                    IQueryable<Users> query = julius2DB.Users.Include(u => u.Post);

                    listUser = julius2DB.Users.Where(c => c.user.StartsWith("v")).ToList();

                    foreach (var userAux in listUser)
                        lk.Add(userAux);
            ...
            catch (Exception e)
            {
                ...
                answer.errors = "Error al momento de consultar el usuario: " + e.Message;
